...
object ComparisonGenerator {
  ...
  def loadingSandbox[T <: CanTag](implicit m: scala.reflect.Manifest[T]): (Double, CanTag) = {
    ...
    val (start, tagger, end) = if(cls.getCanonicalName.contains("etri")){
      val apiKey = System.getenv("API_KEY")
      val start = System.currentTimeMillis()
      val tagger = m.runtimeClass.getConstructor(classOf[String], classOf[String]).newInstance(apiKey, "wsd").asInstanceOf[T]
